...
Select Rank() over(order by score desc) AS Student_Rank,
Name,
Score,
Course
...
(LAG(score) OVER (ORDER BY score DESC) - score) AS score_gap
FROM Student_Performance;
...
ROW_NUMBER() OVER (
PARTITION BY course 
ORDER BY attendance DESC) AS attendance_rank_in_course
FROM Student_Performance;
